...
- `limimeshi-android`: 一般向けAndroidアプリ（Kotlin + Jetpack Compose）※Phase2で実装
- `limimeshi-web`: 一般向けWebアプリ ※Phase3以降
- `limimeshi-jobs`: 定期処理

## 現在のフェーズ

**Phase2: MVP実装**（未着手、準備完了）
...
### 1-3. ADRで技術選定記録 ✅ 完了
- `adr/001-use-firebase-for-backend.md`：Firebase選定理由
- `adr/002-adopt-multi-repository-structure.md`：マルチリポジトリ選定
- `adr/003-use-react-admin-for-admin-panel.md`：React Admin選定
- `adr/004-use-manual-data-entry-for-phase2.md`：完全手動運用
- `adr/005-deploy-using-firebase-hosting-multi-site.md`：Firebase Hosting構成

### 1-4. データモデル詳細設計 ✅ 完了
- `data-model/firestore-collections.md`：コレクション設計
