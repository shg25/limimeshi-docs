...
- 参考: [React公式](https://react.dev/learn/thinking-in-react)
...
## フェーズ5: ユーザーストーリー2 - キャンペーンの管理 (優先度: P1) 🎯 MVP

**目標**: 管理者がキャンペーンを手動で登録し、販売開始日時と販売終了日時を管理できる

**独立テスト**: キャンペーンの登録・一覧表示・編集・削除が完全に動作し、チェーン店との紐付けが正しく機能する
...
- [ ] T041 [P] [US2] limimeshi-admin/src/campaigns/StatusField.tsx で StatusField コンポーネントを作成（計算されたステータスを表示）
- [ ] T042 [P] [US2] limimeshi-admin/src/campaigns/CampaignList.tsx で CampaignList コンポーネントを作成（List, Datagrid, ReferenceField, DateField, StatusField）
- [ ] T043 [P] [US2] limimeshi-admin/src/campaigns/CampaignEdit.tsx で CampaignEdit コンポーネントを作成（Edit, SimpleForm, ReferenceInput, TextInput, DateTimeInput）
- [ ] T044 [P] [US2] limimeshi-admin/src/campaigns/CampaignCreate.tsx で CampaignCreate コンポーネントを作成（Create, SimpleForm, ReferenceInput, TextInput, DateTimeInput）
- [ ] T045 [US2] CampaignList のデータ変換にステータス計算を統合（各キャンペーンに status フィールドを追加）
- [ ] T046 [US2] CampaignList に saleStartTime 降順ソートを追加（sort={{field: 'saleStartTime', order: 'DESC'}}）
- [ ] T047 [US2] limimeshi-admin/src/App.tsx で campaigns リソースを設定（Resource name="campaigns", list, edit, create）
- [ ] T048 [US2] CampaignList にキャンペーン名とチェーン名での検索フィルタを追加（filters プロップ）
- [ ] T049 [US2] CampaignList に確認ダイアログ付き削除機能を追加（DeleteButton コンポーネント）
- [ ] T050 [US2] CampaignEdit と CampaignCreate に saleEndTime > saleStartTime のバリデーションを追加
- [ ] T051 [US2] 必須項目（chainId, name, saleStartTime）でのキャンペーン作成をテスト - 手動検証
- [ ] T052 [US2] キャンペーン編集と削除をテスト - 手動検証
- [ ] T053 [US2] 様々な日付シナリオでのステータス自動計算をテスト - 手動検証
- [ ] T054 [US2] saleEndTime <= saleStartTime 時のバリデーションエラーをテスト - 手動検証

**チェックポイント**: キャンペーン管理が完了し、独立して機能する
...
- [ ] T057 [P] limimeshi-admin/tests/e2e/campaigns.spec.ts でキャンペーン CRUD 操作の E2E テストを追加（Playwright）
- [ ] T058 [P] limimeshi-admin/tests/integration/ChainList.test.tsx で ChainList の統合テストを追加（React Testing Library）
- [ ] T059 [P] limimeshi-admin/tests/integration/CampaignList.test.tsx で CampaignList の統合テストを追加（React Testing Library）
- [ ] T060 パフォーマンス最適化: 必要に応じて CampaignList にページネーションを追加（デフォルト perPage: 25）
...
- **ユーザーストーリー2（P1）**: US3 完了後に開始可能 - US1 に弱い依存（キャンペーン作成にはチェーン店の存在が必要だが、US1 コンポーネントの完成は不要）
...
6. フェーズ5完了: ユーザーストーリー2 → キャンペーン管理動作
...
3. US2 追加 → 管理者がキャンペーンを管理可能（完全なMVP！）
4. 仕上げ追加 → 本番品質
...
3. US3 完了後:
   ...
   - 開発者B: ユーザーストーリー2（キャンペーン）- 並列実行可能だが、テストには少なくとも1つのチェーン店が必要
4. フェーズ6: 仕上げを全員で実施
