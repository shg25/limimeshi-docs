...
- [ ] T009 Kotlinデータクラス定義を作成 data/model/Chain.kt、Campaign.kt、CampaignStatus.kt、ChainWithCampaigns.kt
...
**Goal**: 一般ユーザーがチェーン店を一覧表示し、各チェーン店に紐づくキャンペーン情報をチェーン店単位で確認できる。

**Independent Test**: チェーン店の一覧表示が正しく動作し、ふりがな順でソートされ、各チェーン店に紐づくキャンペーンが販売開始日時の降順で表示されれば、独立して価値を提供できる。
...
- [ ] T012 [P] [US1] チェーン店一覧表示のUIテストを作成 app/src/androidTest/java/.../ui/chain/ChainListScreenTest.kt（チェーン店がふりがな順で表示、キャンペーンが販売開始日時降順で表示、1年経過キャンペーンは非表示、空リスト時「データなし」表示）

### Implementation for User Story 1

- [ ] T013 [P] [US1] ステータス判定ロジックを実装 util/CampaignStatusUtil.kt（getCampaignStatus関数、現在時刻とsaleStartTime/saleEndTimeを比較）
- [ ] T014 [US1] ChainRepositoryを作成 data/repository/ChainRepository.kt（Firestoreからチェーン店一覧をふりがな順で取得、各チェーン店に紐づくキャンペーンを1年以内・販売開始日時降順で取得）
- [ ] T015 [US1] ChainListViewModelを作成 ui/chain/ChainListViewModel.kt（UIステート管理、チェーン店一覧取得、エラーハンドリング）
- [ ] T016 [US1] チェーン店一覧画面を作成 ui/chain/ChainListScreen.kt（LazyColumn、ローディング状態、エラー状態、空リスト状態）
...
- [ ] T023 [P] [US2] フィルタ選択永続化の単体テストを作成 app/src/test/java/.../data/repository/PreferencesRepositoryTest.kt（DataStoreへの保存・読み込み、デフォルト値の処理）
- [ ] T024 [P] [US2] お気に入りフィルタのUIテストを作成 app/src/androidTest/java/.../ui/chain/ChainListScreenTest.kt（フィルタON→お気に入りチェーン店のみ表示、フィルタOFF→全チェーン店表示、次回起動時もフィルタ設定保持）

### Implementation for User Story 2

- [ ] T025 [P] [US2] PreferencesRepositoryを作成 data/repository/PreferencesRepository.kt（DataStore Preferences、showFavoritesOnly保存・読み込み）
...
- [ ] T030 [US2] フィルタ選択永続化を統合 ui/chain/ChainListViewModel.kt（初回起動時にDataStoreから設定を読み込み、フィルタ変更時に保存）
...
### ユーザーストーリーごとのタスク数:
- **User Story 1（チェーン店一覧の閲覧）**: 12タスク（テスト2 + 実装10）
- **User Story 2（お気に入りチェーン店フィルタ）**: 9タスク（テスト2 + 実装7）

### 各ストーリーの独立テスト基準:
- **User Story 1**: チェーン店の一覧表示が正しく動作し、ふりがな順でソートされ、各チェーン店に紐づくキャンペーンが販売開始日時の降順で表示される
- **User Story 2**: お気に入りフィルタが正しく動作し、お気に入り登録済みチェーン店のみが表示され、フィルタ選択が次回訪問時も保持される
