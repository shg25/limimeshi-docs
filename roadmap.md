...
3. ⏭️ 001-admin-panel: tasks.md生成（`/speckit-tasks`）
4. ⏭️ ADR-001〜004作成（基本方針の明文化）
5. ⏭️ 002-menu-list, 003-favorites: plan.md, tasks.md生成
...
**実施順序**（依存関係を考慮）:
1. [ ] `adr/001-why-firebase.md`：なぜFirebaseを選んだか（最優先、全ての前提）
2. [ ] `adr/002-monorepo-vs-multirepo.md`：モノレポ vs マルチリポ（リポジトリ構成の根拠）
3. [ ] `adr/003-react-admin.md`：React Adminを選んだ理由（research.mdの内容を移行）
4. [ ] `adr/004-manual-data-update.md`：データ更新の運用方針（完全手動運用の選定理由）
5. [x] `adr/005-firebase-hosting-multi-site.md`：Firebase Hosting構成とインフラ管理戦略（完成）
