...
- キャンペーンの有無で、「今日はここに行こう」と決められる

### このサービスを使う理由
- Google Mapsではキャンペーンまでは分からない
...
- 日本の大手チェーン店で現在開催中の期間限定キャンペーンを一覧できるサービス
  - 例：マクドナルドのグラコロキャンペーン、吉野家の鍋膳フェア、丸亀製麺の牡蠣キャンペーン
  - 1つのキャンペーンには複数のメニューが含まれることがある（グラコロ2種類 + シーズニングなど）
- チェーン店をお気に入り登録して、自分に関連性のあるチェーン店のみの一覧が作成できる
- 利用したキャンペーンの記録とレビューができる（Phase3以降）
  - 複数回利用したことも記録でき、振り返ることができる
    - 例えば、キャンペーン期間中に5回も足を運んでいたら、思い入れがあることが伝わる（点数の評価では表せない）
  - コメントで具体的に食べたメニュー名も言及可能

---

# 追加機能（Phase3以降）

- 位置情報を元に近くにあるチェーン店の期間限定キャンペーンを確認
...
- 所属チェーンID（外部キー）
- キャンペーン名（必須）
- 説明（任意、キャンペーン内のメニュー詳細などを記載可能）
- X Post URL（任意、代表的な1投稿）
- 販売期間（任意）
  - 開始日（任意）
  - 終了日（任意、未定の場合はnull）
- ステータス（予定 / 開催中 / 終了）
...
### 備考
- キャンペーン名以外は任意入力
- 1キャンペーンに複数メニューが含まれる場合は説明欄に記載
- キャンペーンの定義・粒度は運用者の判断に委ねる
...
- キャンペーンID（外部キー）
- 利用日
- コメント（任意、具体的に食べたメニュー名も記載可能）
- 作成日時
- 更新日時

### 備考
- 点数評価は行わない（複数回利用した記録が重要）
- 同じキャンペーンを同じ日に複数回レビュー可能（熱狂的なファン対応）
- 重複チェックなし（実装がシンプル、柔軟性が高い）

### UI上の工夫
- マイページで「このキャンペーンを5回利用してる！」と視覚的に表示
- 利用回数でバッジ表示（🍔×5など）
...
```
Tags (1) ─── (N) Chains ─── (N) Campaigns
                │              │
Users (1) ─── (N) Favorites    │
  │                            │
  └─── (N) Reviews ─────────── (1)  ※Phase3以降
...
- キャンペーン情報はスクレイピングによる自動更新が理想的だが、法的リスクを避けるため、手動運用でスタートする
...
| `limimeshi-jobs` | 定期処理 | Functions（onSchedule） |
| `limimeshi-docs` | 企画・設計・ADR・AI前提 | 公開リポジトリ |
| （将来）`limimeshi-infra` | インフラ設定（Firestore Rules/Indexes） | Phase3で作成予定 |
...
1. リポジトリセットアップ
   ...
   - `limimeshi-android`（Androidアプリ、Kotlin + Jetpack Compose）
   - `limimeshi-infra`（Firestoreルール・インデックス管理）
2. CI/CDパイプライン構築（最優先、後回し厳禁）
   - GitHub Actionsでテスト自動実行
   - デプロイ自動化（Firebase Hosting、Google Play Console）
   - リント・型チェック・テストの全自動化
...
4. 管理画面実装（React Admin）
   ...
   - キャンペーン管理
   - タグ管理
5. Androidアプリ実装（Kotlin + Jetpack Compose）
   - チェーン一覧表示
   - キャンペーン一覧表示
   - チェーン店お気に入り登録
   - 各機能の単体テストを実装と同時に作成
   - **注記**: レビュー機能はPhase3以降に延期
...
### 成果物
- 動作するAndroidアプリケーション（ベータ版）
- 管理画面

### 完了条件
- 16チェーン店のキャンペーンが一覧できる
- チェーン店お気に入り登録が動作する
- 管理画面から手動でキャンペーン登録ができる
- CI/CDパイプラインが構築され、テスト・デプロイが自動化されている
- 単体テスト・UIテストが実装され、カバレッジ目標を達成している
...
- ユーザーからの「販売終了報告」機能の実装を検討（Phase3以降）
- チェーン店との提携により、公式情報の定期提供を目指す
- **注記**: 差分検知システムは Constitution V. Legal Risk Zero（法的リスクゼロ）の方針により取りやめ
...
- 運用効率化の補助ツール（Googleアラート、RSSリーダー、AI補助）を活用
- **注記**: 差分検知システムは Constitution V. Legal Risk Zero（法的リスクゼロ）の方針により取りやめ
